...
function Umarah(props) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchDocuments("/Packages"), fetchDocuments("/Packages_Info")])
      .then(([packagesData, packagesInfoData]) => {
        // Extract the topRated package
        const topRatedPackage = packagesData.find((pkg) => pkg.topRated);

        // Filter out the topRated package from the original data
        const otherPackages = packagesData.filter((pkg) => !pkg.topRated);

        // Construct a new array with the desired order
        const orderedData = [
          ...otherPackages.slice(0, 1),
          topRatedPackage,
          ...otherPackages.slice(1),
        ];

        setPackagesData(orderedData);
        setInfoData(packagesInfoData);
      })
      .catch((error) => {
          ...
          <div className="flex">
            <div className="leftHeroUmarah bg-black w-[75%] h-[37.25rem] text-white pt-[9.438rem] text-center">
              <div className="contentLeftHeroUmarahi translate-x-44">
                ...
                <h2 className="font-poppins font-medium text-[5rem]">
                  {infoData[0]?.year}
                </h2>
              </div>
            </div>
          ...
          <div className="containerPkg max-w-[94.5%] mx-auto flex items-center flex-wrap gap-32 xl:gap-0 justify-center pb-10 pt-20">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, index) => (
                <div
                  key={index}
                  className="contentPkg w-[23.563rem] h-[35rem] mr-8"
                >
                  <Skeleton height={300} />
                  <Skeleton height={300} />
                </div>
